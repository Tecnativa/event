...
class EventSaleCase(SavepointCase):
    ...
    def multi_qty_support(self) -> bool:
        """Know if ``event_sale_registration_multi_qty`` is installed.

        That module conflicts with this one's logic, so we need to know if it
        is installed to make tests pass if running in an integration
        environment.
        """
        return bool(
            self.env["ir.module.module"]
            ._installed()
            .get("event_sale_registration_multi_qty", False)
        )
    ...
    def test_sale_workflow(self):
        ...
        self.event_types.invalidate_cache(["seats_reservation_total"])
        ...
        if self.multi_qty_support():
            # One line with 10 registrations
            self.assertEqual(len(wiz2.event_registration_ids), 1)
            self.assertEqual(wiz2.event_registration_ids.edit(0).qty, 10)
        else:
            # 10 lines with 1 registration
